...
グラフを作成する場面では、リスト`G`の`i`番目の要素は、`i`番目のノードが隣接している1つ下の階層のノードのindexのリストとなっている。要はグラフの隣接リスト表現[^1]である。なお、リストにおけるノードの順番は、上の階層から数え、同じ階層では左から順に数えていくことを想定している。
...
![plot_large_dev.png](Bi3Q6RfH2-plot_large_dev.png)
